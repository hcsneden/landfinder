...
# Deploys the CDK stacks using the "landfinder-admin" AWS CLI profile, which
# this script writes from the KEY= and SECRET= lines in .env.
#
#   ./deploy.sh                                     # all stacks
#   ./deploy.sh LandFinderApiStack                  # named stacks plus dependencies
#   ./deploy.sh --exclusively LandFinderApiStack    # named stacks only
#
# Arguments starting with "-" pass through to cdk deploy. API Gateway CORS
# needs ALLOWED_ORIGINS. When it is not set, the script builds it from the
# LandFinderWebStack CloudFront URL plus the local development origins.
...
# 5. Build the web app when the web stack is being deployed, since the stack
#    uploads whatever is in apps/web/dist.
...
# 8. Deploy. The ${arr[@]+"${arr[@]}"} form keeps an empty array from
#    tripping set -u on the bash 3.2 that ships with macOS.
if [[ ${#STACKS[@]} -eq 0 ]]; then
...
echo ""
echo "Deployment complete. See docs/deployment.md for the post-deploy steps."
